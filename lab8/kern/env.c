...
env_run(struct Env *e)
{
...
#endif /* not VMM_GUEST */
	cprintf("env_run calls unlock_kernel\n");
	unlock_kernel();
	env_pop_tf(&e->env_tf);
#else	/* not lab >= 8 */
	...
	// If you are running a guest environment, you will need to use vmx_vmrun, rather than
	// env_pop_tf to schedule the guest.  Note that, with vmx_vmrun,
	// you should not unlock the kernel here.
	//
	// Further note that this code should still be able to schedule normal environments.

	// LAB 6b: Your code here
	if(e->env_type == ENV_TYPE_GUEST) {
		panic ("env_run does not support guest environments\n");
	}
#endif // Not VMM_GUEST
}
