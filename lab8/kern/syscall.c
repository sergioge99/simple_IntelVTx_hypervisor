	    ...
	    envid_t guest, void* guest_pa, int perm)
{
		...
		//Comprobamos que el entorno fuente existe
		if (r < 0) {
			cprintf("\n Invalid envid");
			return r;
		}
		r = envid2env(guest, &guestenv, 1);
		//Comprobamos que el entorno invitado existe
		if (r < 0) {
			cprintf("\n Invalid guest envid");
			return r;
		}
		//Comprobamos que las direcciones están alineadas y no superan los limites
		...
		//Comprobamos que existe la pagina fuente
		if (!map) {
			cprintf("\n page not found\n");
			return -E_INVAL;
		}
		...
		r = ept_page_insert(guestenv->env_pml4e, map, guest_pa, perm, 0);
		//Comprobamos que el mapeo ha sido correcto
		if (r < 0) {
			cprintf("\n hva to gpa fault\n");
			return r;
		}
...
static envid_t
	...
	int r;
	struct Env *e;
	cprintf("estoy en sys_env_mkguest");
