...
bool vmx_check_ept() {
	/* Your code here */
	if (BIT(read_msr(IA32_VMX_PROCBASED_CTLS), 63) == 1) {//secondary VMX controls
		if(BIT(read_msr(IA32_VMX_PROCBASED_CTLS2), 33) == 1) { //EPT available
...
void asm_vmrun(struct Trapframe *tf) {
	...
	tf->tf_es = 0;
	cprintf("asm_vmrun calls unlock_kernel");
	unlock_kernel();
